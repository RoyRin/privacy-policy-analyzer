...
6. **Data Deletion** — Can users permanently delete their personal data? (max 10 points)
   ...
   - 7: Self-service deletion available but some data may be retained for stated reasons (e.g., legal obligations, fraud prevention) with clear explanation of what is kept and for how long
...
7. **Opt-Out of Non-Essential Data Use** — Can users opt out of non-essential data use including analytics, marketing, profiling, and AI/ML training? (max 10 points)
...
8. **Consent Model** — How does the company obtain and manage consent for data collection? (max 5 points)
   ...
   - 3: Opt-in required for sensitive data categories; other collection assumed by using the service; some revocation mechanism exists
...
9. **Data Portability** — Can users export or transfer their data? (max 5 points)
...
10. **Readability** — Is the policy written in clear, accessible language? (max 5 points)
...
11. **Data Breach Notification** — Does the policy commit to notifying users of data breaches? (max 5 points)
...
12. **Policy Change Notification** — How are users notified of policy changes? (max 5 points)
...
13. **Policy History & Accountability** — Is the policy's revision history available, and does the company describe accountability mechanisms? (max 5 points)
...
14. **Data Minimization** — Does the company commit to collecting only what it needs? (max 8 points)
...
15. **Encryption & Security Practices** — Does the policy describe how user data is protected? (max 7 points)
    ...
    - 5: Mentions encryption for both data at rest and in transit; describes additional security practices (access controls, monitoring) but without naming specific standards or providing independent verification
    - 3: Mentions encryption or security practices in general terms (e.g., "we use encryption," "industry standard security," "state-of-the-art security") without naming specific protocols or standards
    - 1: Vague references to "reasonable safeguards" or "appropriate security measures" with no specifics
    - 0: No mention of security practices

16. **Children's Data** — Does the policy address data collection from minors? (max 5 points)
...
17. **AI & Machine Learning Training** — Does the policy address whether user data is used for AI/ML model training? (max 5 points)
    - 5: Explicitly states user data is NOT used for AI/ML training; OR states AI/ML training is opt-in only with clear controls and the ability to revoke consent; OR the service architecture (e.g., E2E encryption, no server-side access to content) makes training on user content technically infeasible, and the policy's disclosures confirm this architecture
    - 3: AI/ML training use is disclosed with an opt-out mechanism that is clearly described and accessible
    - 1: Mentions AI/ML training or "product improvement" that likely involves ML but offers no user control; OR uses data for AI training with opt-out available only in some jurisdictions or via a complex process
...
18. **International Data Transfers** — Does the policy address cross-border data transfers? (max 5 points)
    ...
    - 3: Discloses that data may be transferred internationally and names the legal basis or safeguards used, but does not specify storage locations or offer residency options
...
Score each question individually using ONLY the levels defined above. Then compute category and overall scores.
...
Respond in EXACTLY this JSON format and nothing else — no preamble, no explanation outside the JSON:
{
  ...
  "categories": {
    ...
    "special_considerations": {"score": <0-10 float, 1 decimal>, "summary": "2-3 sentence summary"}
  },
